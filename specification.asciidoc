...
*効率的な手法*: HTMLBookは次のhttp://www.idpf.org/epub/idx/[EPUB Indexes specification]や、必要に応じて意味の変化を使用できるdata-type属性をもつ索引の見出し語をマークアップするために<ol>/<li> 要素の使用を推奨する。ただしこれは正式な仕様要件ではない。
